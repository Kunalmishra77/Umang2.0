...
const DoctorSearch = () => {
      ...
      <div className="h-64 sm:h-80 lg:h-72 relative shrink-0">
        ...
        <div className="absolute bottom-6 left-6 md:left-8 text-white pr-12 text-left">
          <div className="flex items-center gap-2 mb-2 lg:mb-3">
            <span className="bg-blue-500/30 backdrop-blur-md border border-white/30 text-blue-100 px-3 py-1 rounded-full text-[9px] lg:text-[10px] font-bold uppercase tracking-widest">
              {doc.dept}
            </span>
            <div className="flex items-center gap-1 text-yellow-400 text-[10px] lg:text-xs font-bold">
              <Star className="w-3 h-3 fill-current" /> {doc.rating}
            </div>
          </div>
          <h2 className="text-2xl md:text-4xl lg:text-5xl font-serif font-bold mb-1 leading-tight">{doc.name}</h2>
          <p className="text-blue-200 text-xs lg:text-sm font-bold uppercase tracking-widest">{doc.role}</p>
        </div>
      ...
      <div className="p-6 lg:p-8 flex-1 overflow-y-auto custom-scrollbar pb-32 lg:pb-8">
        <div className="grid grid-cols-3 gap-4 lg:gap-6 mb-8 border-b border-gray-100 pb-8">
          <div className="text-center lg:text-left">
            <p className="text-[9px] lg:text-[10px] text-gray-400 font-bold uppercase mb-1">Experience</p>
            <p className="text-sm lg:text-lg font-bold text-[#0f172a]">{doc.exp}</p>
          </div>
          <div className="text-center lg:text-left">
            <p className="text-[9px] lg:text-[10px] text-gray-400 font-bold uppercase mb-1">Expertise</p>
            <p className="text-sm lg:text-lg font-bold text-[#0f172a] truncate">{doc.tags[0]}</p>
          </div>
          <div className="text-center lg:text-left">
            <p className="text-[9px] lg:text-[10px] text-gray-400 font-bold uppercase mb-1">Rating</p>
            <p className="text-sm lg:text-lg font-bold text-[#0f172a]">{doc.rating}/5</p>
          </div>
        </div>

        <div className="space-y-6 lg:space-y-8">
          <div className="text-left">
            <h4 className="text-[10px] lg:text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-green-500" /> Professional Overview
            </h4>
            <p className="text-gray-600 font-light italic leading-relaxed text-sm lg:text-base">"{doc.about}"</p>
          </div>
          ...
          <div className="bg-blue-50/50 rounded-[2rem] p-6 lg:p-8 border border-blue-100 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-4 w-full sm:w-auto">
              ...
              <div className="text-left">
                <p className="text-[10px] lg:text-xs font-bold text-gray-400 uppercase mb-1">Availability</p>
                <p className="text-sm lg:text-base font-bold text-[#005580]">{doc.nextSlot} (Slots Active)</p>
              </div>
            </div>
            <Link to={`/booking/${doc.id}`} className="w-full sm:w-auto px-6 lg:px-8 py-3 lg:py-4 bg-[#005580] text-white rounded-xl lg:rounded-2xl font-bold text-xs lg:text-sm hover:bg-[#004466] transition-all shadow-lg text-center">
              Instant Booking
            </Link>
          </div>
          
          <Link to={`/doctor/${doc.id}`} className="block w-full py-3 lg:py-4 border-2 border-gray-100 rounded-xl lg:rounded-2xl text-[#0f172a] font-bold text-center hover:bg-gray-50 transition-all text-xs lg:text-sm">
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <section className="bg-[#0f172a] text-white py-16 lg:py-24 relative overflow-hidden">
         ...
         <div className="container-custom relative z-10 text-center max-w-4xl mx-auto px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
               <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif font-bold mb-6 tracking-tight">Find the Right <br /><span className="text-blue-400">Specialist.</span></h1>
      ...
      <section className="py-12 lg:py-24 bg-white">
         <div className="container-custom">
            <div className="bg-white lg:rounded-[3.5rem] lg:shadow-2xl overflow-hidden lg:border border-gray-100 flex flex-col lg:grid lg:grid-cols-12 h-auto lg:h-[800px]">
               
               {/* List Section */}
               <div className="lg:col-span-5 lg:border-r border-gray-100 flex flex-col bg-white overflow-hidden h-[600px] lg:h-full">
                     ...
                     {filteredDoctors.map(doc => (
                        ...
                        >
                           ...
                           <div className="flex-1 min-w-0">
                              <h4 className="font-bold truncate text-sm lg:text-base mb-0.5">{doc.name}</h4>
                              <p className={`text-[9px] lg:text-[10px] font-black uppercase tracking-widest ${selectedDocId === doc.id ? 'text-blue-400' : 'text-[#005580]'}`}>{doc.dept}</p>
                              <div className="flex items-center gap-2 lg:gap-3 mt-1 lg:mt-2 text-[9px] lg:text-[10px] font-bold">
      ...
      {/* FAQ Section */}
      <section className="py-16 lg:py-32 bg-white border-t border-gray-100">
