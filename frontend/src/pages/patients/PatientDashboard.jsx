...
const PatientDashboard = () => {
  const stats = [
    ...
    { label: 'Blood Sugar', value: '110', unit: 'mg/dL', icon: Droplets, color: 'text-primary-600', bg: 'bg-primary-50', trend: '-5%' },
  ...
  const upcomingAppointments = [
    { id: 1, doctor: 'Dr. Ruby Perrin', specialty: 'Cardiology', date: 'Oct 24, 2026', time: '10:00 AM', status: 'Confirmed', img: '/assets/About/Dr. Rakesh Gupta.png' },
    { id: 2, doctor: 'Dr. Darren Elder', specialty: 'Neurology', date: 'Oct 28, 2026', time: '02:30 PM', status: 'Pending', img: '/assets/About/Dr. Rakesh Gupta.png' },
  ];
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <Link to="/patients/book-appointment" className="h-12 px-6 rounded-xl bg-primary-600 text-white font-bold text-sm flex items-center gap-2 hover:bg-primary-700 transition-all shadow-lg shadow-primary-200">
        ...
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${stat.trend === 'Normal' || stat.trend === 'Stable' ? 'bg-emerald-50 text-emerald-600' : 'bg-primary-50 text-primary-600'}`}>
                {stat.trend}
              </span>
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Link to="/patients/appointments" className="text-primary-600 font-bold text-sm flex items-center gap-1 hover:gap-2 transition-all">
          ...
          <div className="space-y-4">
            {upcomingAppointments.map((appt) => (
              <div key={appt.id} className="bg-white p-5 rounded-[2rem] border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:border-primary-200 transition-colors">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-primary-600 mb-2 uppercase tracking-wider">{appt.specialty}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button className="w-10 h-10 rounded-xl bg-slate-100 text-slate-400 flex items-center justify-center hover:bg-primary-600 hover:text-white transition-all">
          ...
          <div className="bg-[#0f172a] rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
            ...
            <div className="relative z-10 max-w-md">
              ...
              <button className="px-6 py-3 bg-white text-[#0f172a] rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-primary-50 transition-colors">
                View Insights
              </button>
            </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Link to="/patients/lab-reports" className="text-primary-600 font-bold text-sm">View All</Link>
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 divide-y divide-slate-100">
            {recentReports.map((report) => (
              <div key={report.id} className="p-6 hover:bg-slate-50 transition-colors first:rounded-t-[2.5rem] last:rounded-b-[2.5rem] group">
                <div className="flex items-center justify-between mb-3">
                  <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center group-hover:bg-primary-600 group-hover:text-white transition-all">
                    <FileText className="w-5 h-5" />
                  </div>
                  <button className="w-8 h-8 flex items-center justify-center text-slate-300 hover:text-primary-600 transition-colors">
                    <Download className="w-4 h-4" />
                  </button>
                </div>
                <h5 className="font-bold text-[#0f172a] text-sm mb-1 group-hover:text-primary-600 transition-colors">{report.title}</h5>
                <div className="flex items-center justify-between">
            ...
            <div className="p-6 bg-slate-50/50 rounded-b-[2.5rem]">
              <button className="w-full py-4 bg-white border border-dashed border-slate-300 rounded-2xl text-slate-400 text-xs font-bold uppercase tracking-widest hover:border-primary-400 hover:text-primary-600 transition-all flex items-center justify-center gap-2">
