...
import { Menu, X, ChevronDown, Phone, Calendar, Heart, Activity, Clock, Mail, ArrowRight, User, BookOpen, Users, Info } from 'lucide-react';
    ...
    { 
      ...
      content: (
        <div className="flex flex-col md:flex-row gap-0 overflow-hidden rounded-3xl">
          <div className="w-full md:w-[32%] bg-gray-50/50 p-6 border-b md:border-b-0 md:border-r border-gray-100">
          ...
          <div className="w-full md:w-[68%] p-8 bg-white">
      ...
      content: (
         <div className="p-2 bg-white">
            {[
               ...
               <Link key={i} to={item.path} className="flex items-center justify-between px-4 py-3 text-[13px] font-semibold text-gray-600 hover:bg-primary-50 hover:text-primary-600 rounded-xl transition-all group">
                  {item.name}
                  <ArrowRight className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-all text-primary-400" />
               </Link>
            ))}
       ...
       content: (
          <div className="p-2 bg-white">
             {[
                ...
                { name: "Health Blogs", slug: "blogs" },
                { name: "Patient Literatur", slug: "patient-information-literature" }
             ].map((item, i) => (
                <Link key={i} to={item.path || `/patient-corner/${item.slug}`} className="flex items-center justify-between px-4 py-3 text-[13px] font-semibold text-gray-600 hover:bg-primary-50 hover:text-primary-600 rounded-xl transition-all group">
                   {item.name}
                   <ArrowRight className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-all text-primary-400" />
                </Link>
             ))}
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${isScrolled ? 'shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1)]' : ''}`}>
        {/* 1. TOP BAR (bg-brand-dark) */}
        <div className={`bg-[#0f172a] text-white transition-all duration-500 overflow-hidden ${isScrolled ? 'max-h-0' : 'max-h-10'}`}>
           <div className="container-custom h-10 flex items-center justify-between text-[11px] font-semibold tracking-wide">
              <div className="flex items-center gap-6">
                 <a href="tel:+918929733550" className="flex items-center gap-2 hover:text-primary-400 transition-colors"><Phone className="w-3.5 h-3.5" /> Emergency: +91 89297 33550</a>
                 <div className="hidden lg:flex items-center gap-2 opacity-70"><Clock className="w-3.5 h-3.5" /> 24/7 Service Available</div>
              </div>
              <div className="flex items-center gap-6">
                 <Link to="/doctors" className="hover:text-primary-400 transition-colors">Find a Doctor</Link>
                 ...
                 <span className="opacity-20 text-gray-400">|</span>
                 <a href="mailto:Umanghospitalgurugram@gmail.com" className="hover:text-primary-400 transition-colors"><Mail className="w-3.5 h-3.5" /></a>
              </div>
           </div>
        </div>

        {/* 2. MAIN NAV (bg-white) */}
        <div className={`bg-white transition-all duration-500 border-b border-gray-100 ${isScrolled ? 'py-2 md:py-3' : 'py-3 md:py-5'}`}>
          <div className="container-custom flex items-center justify-between gap-4">
            {/* LOGO */}
            <Link to="/" className="flex items-center gap-3 shrink-0 group">
               <div className={`transition-all duration-500 ${isScrolled ? 'h-9' : 'h-11'}`}>
                 <img src="/umang.svg" alt="Umang" className="h-full w-auto object-contain" />
               </div>
               <div className="flex flex-col leading-none">
                 <span className="text-xl md:text-2xl font-bold tracking-tight text-gray-900 group-hover:text-primary-600 transition-colors">
                   Umang<span className="text-primary-600">Hospital</span>
                 </span>
                 <span className="text-[8px] uppercase tracking-[0.25em] font-bold text-gray-400 mt-0.5">Superspeciality</span>
               </div>
            </Link>
              ...
              {navLinks.map((link) => (
                ...
                >
                  <NavLink 
                    to={link.path} 
                    className={({ isActive }) => `
                      px-4 py-2 text-[15px] font-bold rounded-xl transition-all duration-300 flex items-center gap-1 whitespace-nowrap
            ...
            <div className="flex items-center gap-3 shrink-0">
               <Link 
                 to="/doctors" 
                 className={`h-11 px-6 rounded-xl flex items-center gap-2 text-[13px] font-bold transition-all duration-300 ${
                   isScrolled 
               ...
               <button 
                 className="xl:hidden w-11 h-11 flex items-center justify-center rounded-xl bg-gray-50 text-gray-700 border border-gray-100 hover:bg-gray-100 transition-colors" 
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed inset-y-0 right-0 w-[80%] max-w-[400px] bg-white z-[160] shadow-2xl p-6 flex flex-col"
            >
               ...
               <div className="flex-1 space-y-1 overflow-y-auto no-scrollbar">
                  {navLinks.map((link) => (
                    <NavLink key={link.name} to={link.path} onClick={() => setIsMobileMenuOpen(false)} className={({ isActive }) => `block px-5 py-3 rounded-xl text-sm font-bold transition-all ${isActive ? 'bg-primary-50 text-primary-600' : 'text-gray-600 hover:bg-gray-50'}`}>{link.name}</NavLink>
                  ))}
               </div>
